...
class SellPage(wx.Dialog):
    """
    This is SellPage.  
    """
    def __init__(self, Vendor):
        wx.Dialog.__init__(self, None, -1, u"商品出库登记", size=(460, 450))
        self.Centre();
        ...
        self.GoodsID_TextCtrl = wx.TextCtrl(S_panel, 131);
        self.S_GoodsSize_ComboBox = wx.ComboBox(S_panel,72,"",(15,30),wx.DefaultSize,S_SizeList,wx.CB_DROPDOWN|wx.CB_READONLY);
        self.S_GoodsPrice2_Label = wx.StaticText(S_panel, 73, "");
        ...
        self.S_GoodsFinalPrice_Label2 = wx.StaticText(S_panel, 76, u"")
        ...
        self.Bind(wx.EVT_TEXT,self.WhileInputID, id=131)
        ...
        fgs.AddMany([(S_GoodsID_Label,1,wx.EXPAND),(self.GoodsID_TextCtrl,1,wx.EXPAND),
    ...
    def WhileInputID(self, evt):
        GoodsID = str((getattr(self, "GoodsID_TextCtrl").GetValue()).encode('gb2312'));
        if(GoodsID!=""):
            # print GoodsID;
            if(SearchGoods.IDJudge(GoodsID)):
                # print GoodsID;
                GoodsPriceTemp=self.GoodsPricesSelected(GoodsID);
                # print GoodsPriceTemp;
                self.S_GoodsPrice2_Label.SetLabel("%s"%GoodsPriceTemp);
                self.S_GoodsFinalPrice_Label2.SetLabel("%s"%GoodsPriceTemp);
            else:
                self.S_GoodsPrice2_Label.SetLabel("");
                self.S_GoodsFinalPrice_Label2.SetLabel("");
        else:
            self.S_GoodsPrice2_Label.SetLabel("");
                

    def WhileInputCount(self, evt):
        Org_price = str((getattr(self, "S_GoodsPrice2_Label").GetLabel()).encode('gb2312'));
        if(Org_price==""):
            Org_price=0;
        ...
        if(Org_price!=0):
            ...
            if(GoodsCount!=0):
                try:
                    GoodsFinalPrice=str(int(float((int(Org_price)*GoodsCount)/100)));
                except:
                    pass;
                self.S_GoodsFinalPrice_Label2.SetLabel("%s" %GoodsFinalPrice);
            else:
                self.S_GoodsFinalPrice_Label2.SetLabel("%s"%Org_price);

    def GoodsPricesSelected(self, ID):
        ID=str(ID);
        dbreader=shelve.open('database.dat','r')
        if(SearchGoods.IDJudge(ID)):
    ...
    def Submit_QuerySell(self, evt):
        try:
            ...
            SearchGoods.ReminderMessageBox(u"数据有误，请检查商品编号是否正确！")

    def Reset_QuerySell(self, evt):
        getattr(self,"GoodsID_TextCtrl").SetValue("");
        getattr(self,"S_GoodsSize_ComboBox").SetStringSelection("");
        getattr(self, "S_GoodsVendor_ComboBox").SetStringSelection(self.S_Vendor_Default);
        getattr(self,"GoodsCount").SetValue("");
        self.S_GoodsFinalPrice_Label2.SetLabel("");

    # def Print_Contents(GoodsID,GoodsName,GoodsColor,GoodsSize,GoodsVendor,GoodsPrice,GoodsCount,GoodsFinalprice):
    def Print_Contents(self):
        Print_Contents="";
        if(self.PrinterEnable==0):
            Print_Contents="%s/n%s/n/n/n%s:%s/n%s:%s/n%s"%(self.Welcome_Print,self.Name_Print,self.Addr_Print,self.Address_Print,self.TelPhone_Print,self.Tel_Print,self.Comments_Print);
